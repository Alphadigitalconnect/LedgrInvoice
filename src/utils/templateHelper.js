...
import { formatDateDMY } from '../components/Invoices/InvoicePreview';
...
• IFSC: {ifsc}
{upiLine}
Kindly process the payment and share the UTR reference. Copy of Invoice PDF is attached.
...
Please find attached the official copy of Tax Invoice #{invoiceNumber} dated {invoiceDate} for professional services rendered.
...
export const AVAILABLE_TEMPLATE_TAGS = [
  ...
  { tag: '{invoiceDate}', desc: 'Invoice Date (DD-MM-YYYY)' },
  { tag: '{dueDate}', desc: 'Payment Due Date (DD-MM-YYYY)' },
...
export function renderTemplate(template, invoice, entity, client) {
  ...
  const invoiceDate = formatDateDMY(invoice.invoiceDate);
  const dueDate = formatDateDMY(invoice.dueDate);
  const grandTotal = formatINR(invoice.grandTotal || invoice.totalAmount || 0);
  const taxableAmount = formatINR(invoice.taxableTotal || invoice.subtotal || invoice.taxableAmount || 0);
  const gstAmount = formatINR((invoice.totalTax || invoice.totalTaxAmount || (invoice.cgstAmount || 0) + (invoice.sgstAmount || 0) + (invoice.igstAmount || 0)) || 0);
  ...
  const upiLine = upiId ? `• UPI ID: ${upiId}` : '';
  ...
  result = result.replace(/{upiLine}/g, upiLine);
